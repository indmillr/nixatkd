...
const FormsSlider = () => {
  ...
  // Function to check if a form should be rendered for the user
  const shouldRenderForm = (formRole) => {
  ...
  return (
    <Swiper
      ...
      className="h-full"
    >
      {filteredFormsData.map((pattern, index) => (
        <SwiperSlide key={index} className="mb-6">
          <div className="flex h-full flex-col items-center gap-x-2 px-8 mt-0">
            ...
            <div className="w-full flex-1 flex flex-col items-center px-3 mt-1 before:w-[1px] mb-5 relative">
              ...
              <div
                className={`text-left w-[300px] max-h-[375px] overflow-y-auto rounded-xl shadow-md p-3 bg-white dark:bg-black ${
                  pattern.title === "Jee-Goo" ? "text-xs leading-relaxed" : ""
                }`}
              >
                {pattern.meaning}
              </div>
